...
def main(inputtable_pcread, inputtable_for_output, outputdir, outputtable):
	...
	for col in barcode_col:
		...
		megatable_output.insert(nmbr_scc, col + '_REAL_NUM_BARCODE', [0 for x in range(len(megatable))])
		megatable_output.insert(nmbr_mask, col + '_BARCODE_MASK', [','.join(str(0) for y in range(len(barcode_col))) for x in range(len(megatable))])
	for index, row in log_progress(megatable.iterrows(), name = inputtable_pcread, every = 250, size = len(megatable)):
		scc = []
		barcode_cluster = []
		for col in barcode_col:
			barcode_list = re.findall('.........', row[col + '_BARCODE_LIST'])

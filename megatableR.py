...
def megaclustering(df, perm_gap, megacluster_id, chrom, strand, table, files_name):
    ...
    for index, row in df.iterrows():
        if not is_cluster_open:
            ...
            if abs(int(row['START']) - start) <= perm_gap:
                ...
                is_cluster_open = False
                table.write(str(megacluster_id) + '\t' + 'chr' + chrom + '\t' + strand + '\t' + 
...
def main(inputdir, outputdir, perm_gap):
    ...
    colnames = ['CLUSTER_ID', 'CHR', 'STRAND', 
...
'\t'.join(x + 'NUM_READS' for x in additional_colnames) + '\t' + '\t'.join(x + 'NUM_BARCODES' for x in additional_colnames) + '\n')
    ...
    for i_chrom in log_progress(range(22), name = 'megatable', every = 1, who = 'classes: chrom'):
        ...
        for name, group in bigdata_group:
            ...
            megacluster_id = megaclustering(group, perm_gap, megacluster_id, str(i_chrom + 1), name, table, list(start_point.keys()))
    table.close()
